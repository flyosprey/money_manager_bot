...
from tbot.clients.walletapp_web.base import MoneyManagerBase
from tbot.dto.walletapp_web.type import Category, RecordType, SubCategoryFood
...
class MoneyManager(MoneyManagerBase):
    ...
    def set_category(self, category: Category, sub_category: Category) -> None:
    ...
    def press_add_record(self) -> None:
        ...
        self.start_requests_index = len(self.driver.requests)
        add_record.click()
    ...
    def select_category(
        ...
        xpath = f".//div[@class='icon-option category-option']//div[contains(text(), '{category.value}')]"
        time.sleep(1)
        category_category = WebDriverWait(self.driver, 20).until(
