...
def check_walletapp_credentials(username: str, password: str, base_url: str, user_id: int):
    if get_user_state(user_id=user_id) == UserStates.AWAITING_WALLETAPP_PASSWORD:
        ...
        with MoneyManager(username=username, password=password, base_url=base_url) as manager:
            manager.login()
